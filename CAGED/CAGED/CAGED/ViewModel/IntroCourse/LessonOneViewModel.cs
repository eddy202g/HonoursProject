...
{
    public class LessonOneViewModel : INotifyPropertyChanged
    {
        ...
        #region Constructor
        public LessonOneViewModel(int lessonValue)
        {
            ...
            CorrectAnswerBtnText = "Root Note";
            CorrectAnswerBtnCommand = new RelayCommand(correctAnswerBtn);

            WrongAnswerBtnText = "First String";
        ...
        {
            LessonOneModalView testModal = new LessonOneModalView("TEST", this);
        ...
        private void PopulateElements(int lessonValue)
        {
            ...
            using (var reader = new System.IO.StreamReader(stream))
            {
                ...
                {
                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 0)
                    {
                        ScreenOneparaOne = rootObject.paraOne;
                        ScreenOneparaTwo = rootObject.paraTwo;
                    }

                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 1)
                    {
                    ...
                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 3)
                    {
                        ScreenThreeparaOne = rootObject.paraOne;
                        SetImageOne = rootObject.findImage;
                    }

                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 4)
                    {
                    ...
                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 5)
                    {
                        ScreenFiveparaOne = rootObject.paraOne;
                        ScreenFiveparaTwo = rootObject.paraTwo;
                    }

                    if (lessonValue == 1 && rootObject.lessonID == 1 && rootObject.pageID == 6)
                    {
        ...
        public void UpdateDatabase()
        {
            using (var conn = new SQLiteConnection(App.DatabaseLocation))
            {
                ...
                var updateCurrentUser = conn.ExecuteScalar<RegisterModel>("UPDATE RegisterModel SET LessonOneProgress = ? WHERE CurrentUser = ?", "true", "true");
